...
export class UserProfileApi {
    ...
    async getTeachingStaffProfileWithId(id): Promise<TeachingStaffProfileInfo> {
        const token = document.cookie.split("=")[1];
        const headers: HeadersInit = { "Authorization": `Bearer ${token}` };
        const requestOptions: RequestInit = { mode: 'cors', headers: headers, method: "get", credentials: "include" };
        const response: Response = await fetch(`http://localhost:8082/Profile/getTeacherProfile/${id}`, requestOptions);
        return await response.json();
    }
